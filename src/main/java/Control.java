...
public class Control {
    ...
    public static boolean[][] genBoard() {
        boolean[][] board = new boolean[10][17];

        for (int i = 0; i < 17; i++)
            for (int j = 0; j < 10; j++)
                board[j][i] = false;

        board[3][0] = board[4][0] = board[5][0] = board[6][0] = true;
    ...
    public static void main(String[] args) throws Exception {
        ...
        Theory theory = new Theory(new FileInputStream("/home/cerber/prolog-tryout/src/main/java/TetrisProlog.pro"));
        ...
        while(!gameLost) {
            ...
            boolean[][] board = genBoard();
            //readScreen(screenImage);
			for (int i = 0; i < 17; i++) {
				for (int j = 0; j < 10; j++) {
					if (board[j][i])
						System.out.print("X");
